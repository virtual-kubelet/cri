...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	if c.Provider != "" && c.Provider != "cri" {
		return errdefs.InvalidInputf("provider %q not supported: only the cri provider is supported", c.Provider)
	}

	p, err := cri.NewCRIProvider(
		c.NodeName,
		c.OperatingSystem,
